...
float ACleverAIController::ComputeHouseScore(float Distance, float TimeLeft) const
{
	...
	return -Distance * (TimeLeft * TimeLeft);
}
...
void ACleverAIController::Tick(float DeltaSeconds)
{
	...
	if (bDeliveringOrder) {
		...
		for (int i = 0; i < Orders.Num(); ++i) {
			if (Orders[i].OrderNumber != CurrentOrderNumber) {// && !ServedOrders.Contains(Orders[i].OrderNumber)) {
	...
	for (int i = 0; i < Orders.Num(); ++i) {
		...
		if (TimeLeft < 10 && Distance < 600.0f) {
			BestOrder = i;
			neighbour = true;
		}

		if (!neighbour && (BestOrder == -1 || Score > BestScore)) {
			BestScore = Score;
			BestOrder = i;
		}
	}
